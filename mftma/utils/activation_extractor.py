...
def extractor(model, data, layer_nums=None, layer_types=None, return_tensors=True):
    '''
    ...
    Args:
        ...
        return_tensors (optional): If True, keep activations as PyTorch tensors. If False,
            convert to numpy arrays (default: True)
    ...
    registered_hooks = register_hooks(flat_children, return_tensors)

    extracted_dict['layer_0_Input'] = []
    for d in data:
        # Store the input
        if return_tensors:
            extracted_dict['layer_0_Input'] += [d]
        else:
...
def activation_extractor(self, input, output):
    ...
    if hasattr(self, 'return_tensors') and self.return_tensors:
        extracted_dict[layer_name] += [output.detach()]
    else:
...
def register_hooks(flat_children, return_tensors=True):
    '''
    Register the activation extraction hook on each of the given layers
    '''
    registered_hooks = []
    for child in flat_children:
        # Set flag for whether to return tensors or numpy arrays
        child.return_tensors = return_tensors
